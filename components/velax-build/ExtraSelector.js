...
const Item = ({
  ...
  const variantIndex =
    handle === "bagageiro-traseiro" ? rackmap[model.size] : 0;
...
const ExtraSelector = ({ collection, onSelect, selected, model }) => {
  ...
  return (
    <>
      ...
      <p>
        Adicione todos os ítens extras essenciais para Velejar com tranquilidade
        pela cidade:
      </p>
      <div className="extraitems">
        {collection.products.edges
          ...
          .map((product) => {
            if (
              model.size != "P" &&
              product.node.handle === "bagageiro-dianteiro-1"
            )
              return;
            return (
              <Item
